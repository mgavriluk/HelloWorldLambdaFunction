    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(Dictionary<string, object> request, ILambdaContext context)
    {
        ...
        try
        {
            switch (httpMethod)
            {
                ...
                default:
                    return new APIGatewayProxyResponse { StatusCode = (int)HttpStatusCode.BadRequest, Body = "Invalid request." };
            }
        }
        catch (Exception ex)
        {
            context.Logger.LogError($"Error: {ex.Message}");
            return new APIGatewayProxyResponse { StatusCode = (int)HttpStatusCode.InternalServerError, Body = "Server error." };
        }
    }
